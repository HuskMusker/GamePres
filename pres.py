...
/* Кнопка "Назад" в nav-container (первая колонка) */
...
INIT_STATE = {
    ...
    **{f"m13_sort_idx_{i}": 0 for i in range(1, 9)},  # индексы 0-5 (0: не выбрана)
...
def render_step_indicator(module_name, current, total):
    """Отображает индикатор текущего шага с прогресс-баром"""
    pct = (current + 1) / total
    st.markdown(
        f'''
...
def render_nav_buttons(back_step, next_step, next_disabled, back_label="⬅️ Назад", next_label="Далее ➡️"):
    """Универсальная функция для кнопок навигации"""
...
def calculate_auto_score():
    """Расчёт автоматических баллов (без открытых заданий)"""
    score_12 = 0
...
def generate_csv_data():
    """Собирает основные ответы для экспорта"""
    data = {
        "Вопрос": [],
        "Ответ": []
    }
...
if st.session_state.page == "intro":
    st.markdown('<div class="main-header">Добро пожаловать на курс «Цифровой наставник»!</div>', unsafe_allow_html=True)
    st.markdown("""
    <div class="theory-card">
      <p>Этот курс поможет вам освоить навыки цифрового куратора – специалиста, который помогает людям безопасно пользоваться интернетом и защищать свои данные.</p>
      <p>Курс состоит из двух модулей:</p>
      <ul>
        <li><strong>1.2 Нормативно-правовая база и этика</strong> – вы узнаете о законах, регулирующих защиту данных, правах клиентов и этических принципах работы.</li>
        <li><strong>1.3 Информационная безопасность и защита персональных данных</strong> – вы научитесь распознавать угрозы, создавать надёжные пароли, настраивать приватность и составлять рекомендации для клиентов.</li>
      </ul>
      <p>В конце курса вас ждёт диагностика, которая проверит знания из обоих модулей и определит ваш уровень.</p>
...
# ---------- МОДУЛЬ 1.2 ----------
    ...
    if step == 0:
        ...
        <div class="bridge-block">
          <strong>📌 Мостик к разделу 1.3:</strong> Мы вернёмся к этому примеру в разделе 1.3, когда будем учиться защищать данные на практике.
        </div>
        """, unsafe_allow_html=True)
        ...
        if st.button("✅ Проверить", key="check_step1"):
            ...
            if score == 5:
                st.success("🎉 Отлично! Вы верно определили всех участников цифрового пространства.")
            elif 3 <= score <= 4:
                st.info(f"👍 Вы верно определили большинство терминов ({score} из 5). Обратите внимание на пары, отмеченные красным.")
            else:
                st.warning(f"📚 Вам стоит повторить материал. Вы правильно определили только {score} из 5.")
        st.markdown("""
        <div class="bridge-block">
          <strong>📌 Мостик к разделу 1.3:</strong> Запомните, кто такой пользователь — это ваши будущие клиенты, чьи данные вы будете защищать в разделе 1.3.
        </div>
        """, unsafe_allow_html=True)
        ...
        st.write("Представьте, что к вам пришёл клиент и просит удалить его данные. Какие права у него есть? Напишите краткий ответ (3–5 предложений).")
        reflection = st.text_area("Ваш ответ:", key="step2_reflection", height=80)
        st.markdown("""
        <div class="bridge-block">
          <strong>📌 Мостик к разделу 1.3:</strong> В разделе 1.3 мы разберём, как технически обеспечить удаление данных и какие инструменты для этого существуют.
        </div>
        """, unsafe_allow_html=True)
        all_answered = all([
        ...
        if st.session_state.step3_q1 is not None:
            if st.session_state.step3_q1 == "B":
                st.success("✅ Это и этичное, и профессиональное решение. Вы заботитесь о безопасности клиента, но не навязываете своё мнение.")
            elif st.session_state.step3_q1 == "A":
                st.error("❌ Это неэтичное решение. Вы знаете о риске, но не предупреждаете клиента, нарушая принцип «не навреди».")
            else:
                st.error("❌ Это неэтичное и потенциально незаконное решение. Вы не получили информированного согласия клиента на изменение пароля.")
        ...
        st.markdown("""
        <div class="bridge-block">
          <strong>📌 Мостик к разделу 1.3:</strong> Этот кейс мы разберём на практике в разделе 1.3. Подготовьте свои вопросы по защите данных и созданию надёжных паролей.
        </div>
        """, unsafe_allow_html=True)
        ...
        <div class="bridge-block" style="border-left-color: var(--accent-green);">
          <strong>🔗 Мост к разделу 1.3</strong>
        ...
        st.markdown("""
        <div class="theory-card">
          <p>Вы успешно освоили нормативно-правовую базу и этические принципы работы цифрового куратора.</p>
          <p>Теперь вы готовы перейти к практическому модулю <strong>«Информационная безопасность и защита персональных данных»</strong>.</p>
          <p>В этом модуле вы научитесь:</p>
          <ul>
...
# ---------- МОДУЛЬ 1.3 ----------
    ...
    if step == 0:
        ...
        st.markdown("""
        <div style="background: rgba(255,255,255,0.03); border-radius: 16px; padding: 2rem; text-align: center; border: 1px dashed var(--border-card);">
          <div style="font-size: 2rem; margin-bottom: 0.5rem;">▶️</div>
          <p style="color: var(--text-muted);">Анимация «Вспомним законы» будет вставлена здесь</p>
          <span style="font-size:0.9rem; color: var(--text-muted);">(статичная заглушка)</span>
        </div>
        """, unsafe_allow_html=True)
        ...
        # Категории с учётом "Не выбрана" (индекс 0)
        ...
        st.markdown("""
        <div class="theory-card">
          <p>Теперь ты — наставник. Твоя задача — помочь клиенту понять, как настроить приватность в его соцсетях и мессенджерах.</p>
        ...
        if not st.session_state.m13_memo_sent:
            ...
            with st.form("memo_form"):
                ...
                step1_what = st.text_area("Что нужно сделать:", key="m13_memo_step1_what", height=60, placeholder="Опишите действие")
                step1_why = st.text_area("Почему это важно (ссылка на закон/этику):", key="m13_memo_step1_why", height=60, placeholder="Обоснование")
                st.markdown("**Шаг 2. Скрыть дату рождения**")
                step2_what = st.text_area("Что нужно сделать:", key="m13_memo_step2_what", height=60, placeholder="Опишите действие")
                step2_why = st.text_area("Почему это важно (ссылка на закон/этику):", key="m13_memo_step2_why", height=60, placeholder="Обоснование")
                st.markdown("**Шаг 3. Скрыть геолокацию**")
                step3_what = st.text_area("Что нужно сделать:", key="m13_memo_step3_what", height=60, placeholder="Опишите действие")
                step3_why = st.text_area("Почему это важно (ссылка на закон/этику):", key="m13_memo_step3_why", height=60, placeholder="Обоснование")
                st.markdown("**Шаг 4. Скрыть список друзей**")
                step4_what = st.text_area("Что нужно сделать:", key="m13_memo_step4_what", height=60, placeholder="Опишите действие")
                step4_why = st.text_area("Почему это важно (ссылка на закон/этику):", key="m13_memo_step4_why", height=60, placeholder="Обоснование")
                st.markdown("**Шаг 5. Проверить, какие фото видны другим**")
                step5_what = st.text_area("Что нужно сделать:", key="m13_memo_step5_what", height=60, placeholder="Опишите действие")
                step5_why = st.text_area("Почему это важно (ссылка на закон/этику):", key="m13_memo_step5_why", height=60, placeholder="Обоснование")
                ...
                if submitted:
                    ...
                    st.success("✅ Памятка отправлена на проверку наставнику. Вы можете перейти к следующему блоку.")
                    st.rerun()
        else:
        ...
        if not st.session_state.m13_case_sent:
            ...
            st.markdown("#### Выберите вариант действий:")
            case_options = {
            ...
            if st.session_state.m13_case_choice:
                if choice == "C":
                    st.success("✅ Отлично! Вы правильно оценили ситуацию, дали чёткий план и проявили эмпатию.")
                elif choice == "B":
                    st.info("ℹ️ Вы правильно определили угрозу, но не хватает немедленных шагов (блокировка карты).")
                elif choice == "A":
                    st.error("❌ Недостаточно! Вы успокаиваете, но не даёте действий. Это нарушает принцип «не навреди».")
                else:
            ...
            justification = st.text_area("Ваше обоснование (3–5 предложений):", key="m13_case_justification", height=100, placeholder="Введите обоснование...")
            if st.button("📤 Отправить на проверку наставнику", key="send_case"):
                st.session_state.m13_case_sent = True
                st.success("✅ Ваш ответ отправлен на проверку. Вы можете перейти к следующему блоку.")
                st.rerun()
        else:
        ...
        if not st.session_state.m13_project_sent:
            ...
            project_text = st.text_area("Введите текст вашего пакета рекомендаций:", height=400, key="m13_project_text", placeholder="Начните писать...")
            if project_text:
            ...
            if st.button("📤 Сохранить проект и отправить на проверку наставнику", key="send_project"):
                ...
                st.success("✅ Ваш проект сохранён и отправлен на проверку. Вы можете перейти к диагностике.")
                st.rerun()
        else:
            ...
            with st.expander("💡 Пример структуры хорошего пакета"):
                ...
                **Памятка по безопасности для бабушки Варвары**
                1. Пароли: используйте фразу из 4 слов + цифры, например, «КотМурзик2023ЛюбитРыбу!»
                2. Приватность: в ВКонтакте закройте профиль от посторонних.
                ...
        ...
        if st.button("🔍 Проверить диагностику", key="check_diagnostic"):
            ...
            if total_diag >= 10:
                st.success("🎉 Отлично! Вы показали высокий уровень знаний и умений.")
            elif total_diag >= 7:
                st.info("👍 Хороший результат. Обратите внимание на пробелы в ответах.")
            else:
...
# ---------- ИТОГОВАЯ ДИАГНОСТИКА ----------
    ...
    if percent >= 0.7:
        ...
        advice = "Вы отлично владеете правовыми и практическими аспектами цифровой безопасности."
    elif percent >= 0.4:
        level = "🎖️ Уверенный куратор"
        advice = "У вас хорошие знания, но есть пробелы. Повторите темы, связанные с фишингом, социальной инженерией и настройками приватности."
    else:
        level = "🏅 Начинающий куратор"
        advice = "Вам стоит вернуться к материалам обоих модулей и пройти курс заново для закрепления."
    ...
    st.markdown("""
    - Вы можете сохранить свои ответы и проекты для портфолио.
    - Рекомендуем применять полученные знания на практике с реальными клиентами.
    - Следите за обновлениями в области кибербезопасности и регулярно повышайте квалификацию.
    """)

    # Опциональный таймер итоговой диагностики
    with st.expander("⏱️ Таймер (для самоконтроля)"):
        timer_on = st.checkbox("Включить таймер", value=st.session_state.timer_active, key="diag_timer_checkbox")
        if timer_on:
            if not st.session_state.timer_active or st.session_state.timer_start == 0:
                st.session_state.timer_start = time.time()
                st.session_state.timer_active = True
            ...
            if st.button("Отключить таймер", key="diag_timer_off"):
                st.session_state.timer_active = False
                st.rerun()
        else:
            st.session_state.timer_active = False
    ...
    with col1:
        if st.button("🔄 Пройти диагностику заново"):
            st.session_state.m13_diag_score = 0
            st.session_state.m13_diagnostic_checked = False
            st.rerun()
    with col2:
